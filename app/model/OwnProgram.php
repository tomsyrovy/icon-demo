...
class OwnProgram extends Repository{
    ...
    public function getStats($type){
        switch($type){
            ...
            case "ranking" : {
                return $this->getStatsRanking();
            }
            case "rankingS" : {
                return $this->getStatsRankingS();
            }
        }
    }
    ...
    private function getStatsRanking(){
        $crate = array();
        $crate["title"] = "Hodnocení přednášek z iOS app";
        $crate["captions"] = array("Přednáška", "Průměrné hodnocení<br> (1 = min, 5 = max)", "Počet hodnocení");
        $statement = "
          SELECT tt.id, tt.title as t, avg(r.ranking) as t2, count(r.ranking) as t3
          FROM talk tt
            JOIN ranking r ON tt.id = r.talk_id
          WHERE r.granted >= '2014-01-01' AND tt.active = 1
          GROUP BY tt.id
    ...
    private function getStatsRankingS(){
        $crate = array();
        $crate["title"] = "Hodnocení řečníků podle přednášek";
        $crate["captions"] = array("Jméno a příjmení", "Průměrné hodnocení jeho přednášek<br> (1 = min, 5 = max)", "Počet hodnocení jeho přednášek");
        $statement = "
          SELECT s.id, concat(s.firstname, ' ', s.lastname) as t, avg(r.ranking) as t2, count(r.ranking) as t3
          FROM talk tt
            JOIN ranking r ON tt.id = r.talk_id
            JOIN speaker_has_talk st ON tt.id = st.talk_id
            JOIN speaker s ON s.id = st.speaker_id
          WHERE r.granted >= '2014-01-01' AND s.active = 1
          GROUP BY s.id
          ORDER BY t2 DESC, t3 DESC";
        $crate["result"] = $this->connection->query($statement);
